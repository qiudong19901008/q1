...
class GetNeedData{
   ...
   */
  public static function run($query,$includeTableNameList=[],$metaNameList=[]){
    global $post;
    $res = [];
    if(!$query->have_posts()){
      return $res;
    ...
    $res = GetNeedData::_injectOtherFields($res,$includeTableNameList,$metaNameList);
    return $res;
  }
  ...
  private static function _injectOtherFields($myPostList,$includeTableNameList,$metaNameList){
    ...
    //注入meta
    $isIncludeMeta = GetNeedData::_isIncludeMeta($includeTableNameList);
    if($isIncludeMeta){
      $myPostList = GetNeedData::_addMetaTableInfo($myPostList,$metaNameList);
    }
    //注入作者
    $isIncludeAuthor = GetNeedData::_isIncludeAuthor($includeTableNameList);
    if($isIncludeAuthor){
      $myPostList = GetNeedData::_addAuthorTableInfo($myPostList);
    }
    //注入分类
    $isIncludeCategory = GetNeedData::_isIncludeCategory($includeTableNameList);
    if($isIncludeCategory){
      $myPostList = GetNeedData::_addCategoryTableInfo($myPostList);
    }
    //注入标签
    $isIncludeTag = GetNeedData::_isIncludeTag($includeTableNameList);
    if($isIncludeTag){
      $myPostList = GetNeedData::_addTagTableInfo($myPostList);
    }
    return $myPostList;
  }
  ...
  private static function _addCategoryTableInfo($myPostList){
    ...
    foreach($myPostList as $myPost){
      $myPost = GetNeedData::_addCategoryTableInfoToOnePost($myPost);
      array_push($res,$myPost);
    }
    return $res;
  }
  ...
  private static function _addTagTableInfo($myPostList){
    $res = [];
    foreach($myPostList as $myPost){
      $myPost = GetNeedData::_addTagTableInfoToOnePost($myPost);
      array_push($res,$myPost);
    }
    return $res;
  }
  ...
  private static function _addAuthorTableInfo($myPostList){
    $res = [];
    $userIdList = GetNeedData::_getUserIdList($myPostList);
    $userList = UserDao::getUserListByUserIdList($userIdList);
    foreach($myPostList as $myPost){
      $myPost = GetNeedData::_addAuthorTableInfoToOnePost($myPost,$userList);
      array_push($res,$myPost);
    }
    return $res;
  }
  ...
  private static function _addMetaTableInfo($myPostList,$metaNameList=[]){
    $res = [];
    $myPostIdList = GetNeedData::_getPostIdList($myPostList);
    $allMetaList = PostMetaDao::getPostMetaListByPostIdList($myPostIdList,$metaNameList);
    foreach($myPostList as $myPost){
      $myPost = GetNeedData::_addMetaTableInfoToOnePost($myPost,$allMetaList,$metaNameList);
      array_push($res,$myPost);
    }
    return $res;
  }
  ...
  private static function _addMetaTableInfoToOnePost($myPost,$allMetaList,$metaNameList){
    ...
    foreach($metaNameList as $metaName){
      $value = GetNeedData::_extractMetaValueByPostIdAndMetaName($allMetaList,$metaName,$myPost['id']);
